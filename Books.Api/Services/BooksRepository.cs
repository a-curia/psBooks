...
namespace Books.Api.Services
{
    public class BooksRepository : IBooksRepository, IDisposable
    {
        ...
        //    await _context.AddAsync(bookToAdd); // checking the definition shows that if we dont use autogenerate we should not use async
        //}

        public void AddBook(Book bookToAdd) // this method does not work with IO/Db so no need for async
        {
            if (bookToAdd == null)
            {
                throw new ArgumentNullException(nameof(bookToAdd));
        ...
        {
            // return true if 1 or more entities where changed
        ...
        public async Task<IEnumerable<BookCover>> GetBookCoversAsync(Guid bookId)
        {
            ...
            // _cancellationTokenSource.Token this is the token we need to pass to each task to listen for cancellation

            // create a list of fake bookcovers
            var bookCoverUrls = new[]
            {
                $"https://localhost:6001/api/bookcovers/{bookId}-dumm1",
                $"https://localhost:6001/api/bookcovers/{bookId}-dumm2?returnFault=true",
                $"https://localhost:6001/api/bookcovers/{bookId}-dumm3",
                $"https://localhost:6001/api/bookcovers/{bookId}-dumm4",
                $"https://localhost:6001/api/bookcovers/{bookId}-dumm5",
                $"https://localhost:6001/api/bookcovers/{bookId}-dumm6"
            };

            //// call the BookCover API
        ...
        {
            //throw new Exception("Cannot download book cover, write isn't finishing book fast enough.");
            var response = await httpClient.GetAsync(bookCoverUrl, cancellationToken); // if error we want to cancel the download
